...
namespace kdbush {
	...
	template <typename TPoint, typename TNumber, typename TIndex = std::size_t>
	class KDBush 
	{
	public:
		...
		std::vector<TIndex> range(const TNumber minX,
			const TNumber minY,
			const TNumber maxX,
			const TNumber maxY) const 
		{
			std::vector<TIndex> result;
			range(minX, minY, maxX, maxY, [&result](const auto id) { result.push_back(id); });
			return result;
		}
