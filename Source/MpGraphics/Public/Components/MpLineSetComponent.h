...
#include "MpLineSetComponent.generated.h"

class FPrimitiveSceneProxy;

struct FMpRenderableLine
{
	FMpRenderableLine()
	...
	FMpRenderableLine(const FVector& InStart, const FVector& InEnd, const FColor& InColor, const float InThickness, const float InDepthBias = 0.0f)
		: Start(InStart)
		, End(InEnd)
		, Color(InColor)
...
UCLASS()
class UMpLineSetComponent : public UMeshComponent
{
	GENERATED_BODY()
...
public:
	...
	int32 AddLine(const FMpRenderableLine& OverlayLine);
	...
	{
		// This is just a convenience function to avoid client code having to know about FMpRenderableLine.
		return AddLine(FMpRenderableLine(InStart, InEnd, InColor, InThickness, InDepthBias));
	}

	/** Insert a line with the given ID to the overlay */
	void InsertLine(const int32 ID, const FMpRenderableLine& OverlayLine);
...
private:
	...
	TSparseArray<FMpRenderableLine> Lines;

	friend class FMpLineSetSceneProxy;
};