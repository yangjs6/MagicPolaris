...
#include "MpPointSetComponent.generated.h"

class FPrimitiveSceneProxy;


struct FMpRenderablePoint
{
	FMpRenderablePoint()
	...
	FMpRenderablePoint(const FVector& InPosition, const FColor& InColor, const float InSize)
		: Position(InPosition)
		, Color(InColor)
		, Size(InSize)
	{}
...
/**
 * UMpPointSetComponent is a Component that draws a set of points, as small squares.
 ...
 */
UCLASS()
class UMpPointSetComponent : public UMeshComponent
{
	GENERATED_BODY()

public:
	UMpPointSetComponent();
	...
	int32 AddPoint(const FMpRenderablePoint& OverlayPoint);

	/** Insert a point with the given ID into the set. */
	void InsertPoint(const int32 ID, const FMpRenderablePoint& OverlayPoint);

	/** Retrieve a point with the given id. */
	const FMpRenderablePoint& GetPoint(const int32 ID);
...
private:
	...
	TSparseArray<FMpRenderablePoint> Points;

	friend class FMpPointSetSceneProxy;
};
