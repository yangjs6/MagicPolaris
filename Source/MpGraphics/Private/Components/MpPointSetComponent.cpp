...
#include "Components/MpPointSetComponent.h"
...
struct FMpPointSetMeshBatchData
{
	FMpPointSetMeshBatchData()
...
class FMpPointSetSceneProxy final : public FPrimitiveSceneProxy
{
public:

	FMpPointSetSceneProxy(UMpPointSetComponent* Component)
		: FPrimitiveSceneProxy(Component),
		MaterialRelevance(Component->GetMaterialRelevance(GetScene().GetFeatureLevel())),
		VertexFactory(GetScene().GetFeatureLevel(), "FMpPointSetSceneProxy")
	{
		...
		if (Component->Points.Num() > 0)
		{
			...
			FMpPointSetMeshBatchData& MeshBatchData = MeshBatchDatas.Last();
			...
			TArray<const FMpRenderablePoint*> LinearPoints;
			LinearPoints.Reserve(Component->Points.Num());
			for (FMpRenderablePoint& Point : Component->Points)
			{
			...
			{
				const FMpRenderablePoint& Point = *LinearPoints[i];
	...
	virtual void GetDynamicMeshElements(const TArray<const FSceneView*>& Views, const FSceneViewFamily& ViewFamily, uint32 VisibilityMap, FMeshElementCollector& Collector) const override
	{
		...
		for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
		{
			...
			{
				for (const FMpPointSetMeshBatchData& MeshBatchData : MeshBatchDatas)
				{
...
private:
	TArray<FMpPointSetMeshBatchData> MeshBatchDatas;
...
UMpPointSetComponent::UMpPointSetComponent()
{
	CastShadow = false;
	bSelectable = false;
...
void UMpPointSetComponent::SetPointMaterial(UMaterialInterface* InPointMaterial)
{
...
void UMpPointSetComponent::Clear()
{
	Points.Reset();
...
void UMpPointSetComponent::ReservePoints(const int32 MaxID)
{
...
int32 UMpPointSetComponent::AddPoint(const FMpRenderablePoint& OverlayPoint)
{
...
void UMpPointSetComponent::InsertPoint(const int32 ID, const FMpRenderablePoint& OverlayPoint)
{
...
const FMpRenderablePoint& UMpPointSetComponent::GetPoint(const int32 ID)
{
	return Points[ID];
}


void UMpPointSetComponent::SetPointColor(const int32 ID, const FColor& NewColor)
{
	FMpRenderablePoint& OverlayPoint = Points[ID];
	OverlayPoint.Color = NewColor;
	MarkRenderStateDirty();
}


void UMpPointSetComponent::SetPointSize(const int32 ID, const float NewSize)
{
	FMpRenderablePoint& OverlayPoint = Points[ID];
...
void UMpPointSetComponent::SetPointPosition(const int32 ID, const FVector& NewPosition)
{
	FMpRenderablePoint& OverlayPoint = Points[ID];
...
void UMpPointSetComponent::SetAllPointsColor(const FColor& NewColor)
{
	for (FMpRenderablePoint& Point : Points)
	{
...
void UMpPointSetComponent::RemovePoint(const int32 ID)
{
	Points.RemoveAt(ID);
	MarkRenderStateDirty();
	bBoundsDirty = true;
...
bool UMpPointSetComponent::IsPointValid(const int32 ID) const
{
	return Points.IsValidIndex(ID);
}


FPrimitiveSceneProxy* UMpPointSetComponent::CreateSceneProxy()
{
	if (Points.Num() > 0)
	{
		return new FMpPointSetSceneProxy(this);
	}
...
int32 UMpPointSetComponent::GetNumMaterials() const
{
	return 1;
}


FBoxSphereBounds UMpPointSetComponent::CalcBounds(const FTransform& LocalToWorld) const
{
	if (bBoundsDirty)
	{
		FBox Box(ForceInit);
		for (const FMpRenderablePoint& Point : Points)
		{
			Box += Point.Position;
		}
