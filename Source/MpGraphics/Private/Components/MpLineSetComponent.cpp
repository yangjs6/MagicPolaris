...
#include "Components/MpLineSetComponent.h"
...
struct FMpLineMeshBatchData
{
	FMpLineMeshBatchData()
...
/** Class for the MpLineSetComponent data passed to the render thread. */
class FMpLineSetSceneProxy final : public FPrimitiveSceneProxy
{
public:

	FMpLineSetSceneProxy(UMpLineSetComponent* Component)
		...
		VertexFactory(GetScene().GetFeatureLevel(), "FPointSetSceneProxy")
	{
		...
		if (Component->Lines.Num() > 0)
		{
			...
			FMpLineMeshBatchData& MeshBatchData = MeshBatchDatas.Last();
			...
			for (const FMpRenderableLine& OverlayLine : Component->Lines)
			{
				...
				// whereas it is more convenient for the user of the MpLineSet to specify colors as sRGB. So we actually
		...
		ENQUEUE_RENDER_COMMAND(MpLineSetVertexBuffersInit)(
		...
		const FSceneViewFamily& ViewFamily, uint32 VisibilityMap, FMeshElementCollector& Collector) const override
	{
		...
		for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
		{
			...
			{
				for (const FMpLineMeshBatchData& MeshBatchData : MeshBatchDatas)
				{
...
private:
	TArray<FMpLineMeshBatchData> MeshBatchDatas;
...
UMpLineSetComponent::UMpLineSetComponent()
{
	CastShadow = false;
	bSelectable = false;
...
void UMpLineSetComponent::SetLineMaterial(UMaterialInterface* InLineMaterial)
{
	LineMaterial = InLineMaterial;
	SetMaterial(0, InLineMaterial);
}

void UMpLineSetComponent::Clear()
{
	Lines.Reset();
...
void UMpLineSetComponent::ReserveLines(const int32 MaxID)
{
	Lines.Reserve(MaxID);
}

int32 UMpLineSetComponent::AddLine(const FMpRenderableLine& OverlayLine)
{
...
void UMpLineSetComponent::InsertLine(const int32 ID, const FMpRenderableLine& OverlayLine)
{
...
void UMpLineSetComponent::SetLineStart(const int32 ID, const FVector& NewPostion)
{
	FMpRenderableLine& OverlayLine = Lines[ID];
	OverlayLine.Start = NewPostion;
	MarkRenderStateDirty();
	bBoundsDirty = true;
}

void UMpLineSetComponent::SetLineEnd(const int32 ID, const FVector& NewPostion)
{
	FMpRenderableLine& OverlayLine = Lines[ID];
...
void UMpLineSetComponent::SetLineColor(const int32 ID, const FColor& NewColor)
{
	FMpRenderableLine& OverlayLine = Lines[ID];
...
void UMpLineSetComponent::SetLineThickness(const int32 ID, const float NewThickness)
{
	FMpRenderableLine& OverlayLine = Lines[ID];
...
void UMpLineSetComponent::SetAllLinesThickness(const float NewThickness)
{
	for (FMpRenderableLine& Line : Lines)
	{
...
void UMpLineSetComponent::SetAllLinesLength(const float NewLength, bool bUpdateBounds)
{
...
void UMpLineSetComponent::SetAllLinesColor(const FColor& NewColor)
{
	for (FMpRenderableLine& Line : Lines)
	{
		Line.Color = NewColor;
...
void UMpLineSetComponent::RemoveLine(const int32 ID)
{
	Lines.RemoveAt(ID);
	MarkRenderStateDirty();
	bBoundsDirty = true;
...
bool UMpLineSetComponent::IsLineValid(const int32 ID) const
{
	return Lines.IsValidIndex(ID);
}

FPrimitiveSceneProxy* UMpLineSetComponent::CreateSceneProxy()
{
	if (Lines.Num() > 0)
	{
		return new FMpLineSetSceneProxy(this);
	}
	return nullptr;
}

int32 UMpLineSetComponent::GetNumMaterials() const
{
	return 1;
}

FBoxSphereBounds UMpLineSetComponent::CalcBounds(const FTransform& LocalToWorld) const
{
	if (bBoundsDirty)
	{
		FBox Box(ForceInit);
		for (const FMpRenderableLine& Line : Lines)
		{
			Box += Line.Start;
			Box += Line.End;
		}
...
void UMpLineSetComponent::AddLines(
	int32 NumIndices,
	TFunctionRef<void(int32 Index, TArray<FMpRenderableLine>& LinesOut)> LineGenFunc,
	int32 LinesPerIndexHint)
{
	TArray<FMpRenderableLine> TempLines;
	...
	for (int32 k = 0; k < NumIndices; ++k)
	{
		...
		for (const FMpRenderableLine& Line : TempLines)
		{
			AddLine(Line);
		}
	}
